...
def resolve_source_path(path: Path) -> Path:
    """
    Resolve Lightroom-exported source_path through PROETUS_IMAGES_ROOT.

    The Lightroom candidate CSV may contain machine-specific paths, for example:

      /Users/suhail/Mounts/dataLib/Pictures/Images/...

    During travel, the same image tree may live on Orion:

      /Volumes/Orion/Pictures/Images/...

    Rather than rewriting CSVs per machine, keep the CSV stable and remap the
    root at runtime using PROETUS_IMAGES_ROOT.
    """
    if path.exists():
        return path

    images_root_text = os.environ.get("PROETUS_IMAGES_ROOT", "").strip()

    if not images_root_text:
        return path

    images_root = Path(images_root_text).expanduser()
    ...
    for marker in markers:
        if marker in path_text:
            ...
            # Return the remapped path even if it does not exist yet.
            # This makes failures.csv point at the intended current-machine path.
            return candidate
...
def build_previews(
    *,
    ...
    for index, row in enumerate(rows, start=1):
        ...
        source_path = resolve_source_path(Path(source_path_text))
